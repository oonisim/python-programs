...
def test_020_cross_entropy_log_loss_scalar(h: float = 1e-5, e=OFFSET_FOR_LOG):
    ...
    # --------------------------------------------------------------------------------
    # [Scalar test case]
    # For scalar P=1, T=1 for -t * log(1+e) -> log(1+e) : dlog(P+e)/dP -> 1 / (1+e)
    # For scalar P=0, T=1 for -t * log(e) -> log(e)     : dlog(P+e)/dP -> 1 / (e)
    # For scalar P=1, T=0 for -t * log(e) -> 0: loss = log(e) -> dlog(e)/dP -> 0
    # --------------------------------------------------------------------------------
