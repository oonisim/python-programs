...
class Standardization(Layer):
    ...
    def specification_template():
        return Standardization.specification(name="std001", num_nodes=3)
    ...
    def build(parameters: Dict):
        ...
        instance = Standardization(**parameters)
    ...
    def function(
        ...
        # --------------------------------------------------------------------------------
        self._Y, self._U, self._SD, self._Xmd = standardize(
            X,
            eps=self.eps,
            keepdims=False,
            out=self._Y,
    ...
    def _gradient_numpy(self):
        ...
        # --------------------------------------------------------------------------------
        # dL/dV:(M,) = sum(dL/dY:(N,M) * Xmd, axis=0) * [-1/2 * (norm**3)]
        # --------------------------------------------------------------------------------
        np.sum(self.dY * self.Xmd, axis=0, out=self._dV)
        ...
        # --------------------------------------------------------------------------------
        np.multiply(self.dY, self.norm, out=self._dXmd02)
    ...
    def _gradient_numexpr(self):
        ...
        # --------------------------------------------------------------------------------
        # dL/dV:(M,) = sum(dL/dY:(N,M) * Xmd:(N,M), axis=0) * -1/2 * norm**3
        # --------------------------------------------------------------------------------
        ne.evaluate("sum(dY * Xmd, axis=0)", out=dV)
        ...
        # --------------------------------------------------------------------------------
        # dL/dXmd02:(N,M) = dL/dY:(N,M) * norm:(M,)
        # --------------------------------------------------------------------------------
        ne.evaluate("dY * norm", out=dXmd02)
...
class BatchNormalization(Layer):
    ...
    def __init__(
        ...
        # State of the layer
        # --------------------------------------------------------------------------------
        self._S = [self.gamma, self.beta, self.RU, self.RSD]
