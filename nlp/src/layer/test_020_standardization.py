...
def test_020_std_instantiation_to_fail():
    ...
    for _ in range(NUM_MAX_TEST_TIMES):
        ...
        assert \
            ...
            0.0 < _layer.momentum < 1.0, msg

        msg = "initialization with momentum <=0 must fail"
        _must_fail(name=name, num_nodes=M, momentum=np.random.uniform(-1, 0), msg=msg)

        msg = "initialization with momentum > 1 must fail"
        _must_fail(name=name, num_nodes=M, momentum=np.random.randint(1, 100), msg=msg)

        # Constraint: 0 < eps < 1e-3.
        msg = "initialization with eps < 0 must fail"
        _must_fail(name=name, num_nodes=M, eps=np.random.uniform(-100.0, 0), msg=msg)

        msg = "initialization with eps >=1e-3 must fail"
        _must_fail(name=name, num_nodes=M, eps=np.random.uniform(1e-3, 100.0), msg=msg)
...
def test_020_std_instance_properties_access_to_fail():
    ...
    for _ in range(NUM_MAX_TEST_TIMES):
        ...
        # --------------------------------------------------------------------------------
        try:
            if not _layer.name == name: raise RuntimeError("_layer.name == name should be true")
        except AssertionError:
            raise RuntimeError("Access to name should be allowed as already initialized.")

        try:
            if not _layer.M == M: raise RuntimeError("_layer.M == M should be true")
        except AssertionError:
...
def test_020_std_builder_to_succeed():
    ...
    for _ in range(NUM_MAX_TEST_TIMES):
        ...
        eps = valid_std_parameters["eps"]
        momentum = valid_std_parameters["momentum"]
        ...
        try:
            ...
            raise RuntimeError("Matmul.build() must succeed with %s" % valid_std_parameters)

    profiler.disable()
    profiler.print_stats(sort="cumtime")


def _instance(name, num_nodes: int, momentum, eps: TYPE_FLOAT = 0, log_level: int = logging.ERROR):
...
def _validate_layer_values(_layer: Standardization, X, eps):
    ...
    assert np.allclose(_layer.U, X.mean(axis=0), atol=1e-6, rtol=0.0)
    assert np.allclose(_layer.Xmd, md, atol=1e-6, rtol=0.0)
    assert np.allclose(_layer.SD, sd, atol=1e-6, rtol=0.0)
    assert np.allclose(
        _layer.Y,
...
def _validate_layer_running_statistics(
    ...
    if _layer.total_training_invocations == 1:
        ...
        assert np.allclose(_layer.RU, expected_ru, atol=1e-6, rtol=0)
        assert \
            np.allclose(_layer.RSD, expected_rsd, atol=1e-6, rtol=0), \
            "X=\n%s\nX.sd()=\n%s\n_layer.SD=\n%s\n_layer.RSD=\n%s\n" \
...
def test_020_std_method_function_to_succeed():
    ...
    for _ in range(NUM_MAX_TEST_TIMES):
        ...
        for i in range(np.random.randint(1, 100)):
            ...
            assert _layer.total_rows_processed == N * (i + 2)
...
def test_020_std_method_predict():
    ...
    for _ in range(NUM_MAX_TEST_TIMES):
        ...
        expected_RU = _layer.RU * momentum + mean * (1-momentum)
        expected_RSD = _layer.RSD * momentum + sd * (1-momentum)
        Y = _layer.function(
            Z,
            numexpr_enabled=numexpr_enabled
        )
        assert np.allclose(_layer.RU, expected_RU, atol=1e-10, rtol=0)
        assert np.allclose(_layer.RSD, expected_RSD, atol=1e-10, rtol=0)
