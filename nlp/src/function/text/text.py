...
class Function:
    ...
    def word_indexing(corpus: str):
        ...
        probabilities: Dict[int, TYPE_FLOAT] = {word: (count / total) for (word, count) in counts.items()}

        vocabulary: List[str] = ['UNK'] + list(set(words))
    ...
    def sub_sampling(corpus: str, power: TYPE_FLOAT = TYPE_FLOAT(0.75)):
        total = len(words := Function.standardize(corpus).split())
        counts = collections.Counter(words)
